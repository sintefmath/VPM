...
def getFields(pf):
    ...
    reshape = (ny, nx)
    prec=np.float64
    x, y = pf.get_positions_as_numpy()#pyVPM.convert_to_numpy(pf.positions)
    x = np.reshape(x, reshape)
    y = np.reshape(x, reshape)
    
    Ux, Uy = pf.get_velocity_as_numpy()#pyVPM.convert_to_numpy(pf.velocity)
