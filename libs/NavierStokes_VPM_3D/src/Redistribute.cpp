...
namespace VPM
{
    ...
    namespace {
        ...
        inline double interpolation_Kernel_Gamma3(const VPM::Point3d & p, const double delta)
        {
            VPM::Point3d pd = p/delta;
            return Gamma3_dir(pd.x)*Gamma3_dir(pd.y)*Gamma3_dir(pd.z);
        }
        ...
        inline double interpolation_Kernel_M4d(const VPM::Point3d & p, const double delta)
        {
            VPM::Point3d pd = p/delta;
            return M4d_dir(pd.x)*M4d_dir(pd.y)*M4d_dir(pd.z);
        }
        ...
        inline double interpolation_Kernel_M5d(const VPM::Point3d & p, const double delta)
        {
            VPM::Point3d pd = p/delta;
            return M5d_dir(pd.x)*M5d_dir(pd.y)*M5d_dir(pd.z);
        }
        ...
        inline double interpolation_Kernel_M6d(const VPM::Point3d & p, const double delta)
        {
            VPM::Point3d pd = p/delta;
            return M6d_dir(pd.x)*M6d_dir(pd.y)*M6d_dir(pd.z);
        }

        inline double interpolation_Kernel_order6(const VPM::Point3d & p, const double delta)
        {
            VPM::Point3d pd = p/delta;

            double px2 = pd.x*pd.x;
            double py2 = pd.y*pd.y;
            double pz2 = pd.z*pd.z;
            double r2 = px2 + py2 + pz2;
            if (r2>=16)
            {
                return 0;
            }
            ...
            double tmp_z = 15./4. - 5.*pz2 + pz2*pz2;
            double res = 1./(M_PI*sqrt(M_PI))*tmp_x/2.*tmp_y/2.*tmp_z/2.*exp(-r2);
            ...
            )
    {
        ...
        VPM::Point3d scaled_pos = (pos - params->m_domain_ll)/(params->m_domain_ur - params->m_domain_ll);
        ...
        int k_ind = std::floor(scaled_pos.z*double(params->m_num_pz-1));
        ind_min.z = std::max(int(0), k_ind-stenc);
        ind_max.z = std::min(int(params->m_num_pz-1), k_ind+stenc+1);
    }


    void Redistribute::redistribute(
            ParticleField & pf,
            const REDISTRIBUTE3D g2p
            )
    {
        redistribute(
                pf.positions,
                    ...
                    for(unsigned int index_part=0; index_part<params->m_N; index_part++ )
                    {
                        ...
                        else
                        {
                            ...
                            for (unsigned int i=ind_min.x; i<=ind_max.x; i++)
                            {
                                ...
                                {
                                    for (unsigned int k=ind_min.z; k<=ind_max.z; k++)
                                    {
                                        int index_reg = flat(i,j,k, int(params->m_num_px), int(params->m_num_py));
                                        // declaring this const, helps the compiler to optimize (decision outside the for loop(s))
                                        switch (method)
                                        {
                                            case(VPM::Gamma3_fast):
    ...
    template void Redistribute::redistribute<VPM::Point3d>(
            const std::vector<Point3d> & old_pos,
            const std::vector<Point3d> & new_pos,
            std::shared_ptr<Parameters> & params,
            std::vector<VPM::Point3d> & field,
            const REDISTRIBUTE3D g2p
            );
