...
namespace ModulesHotel
{
    ...
    DiffusionEquationSolver3D::DiffusionEquationSolver3D(
            //int argc, char** argv,
            const DiffusionEquationSolver3D_BC boundaryconditions
            )
        :
            m_initialized(false),
            m_N(0)
    {
        m_user.bc_type = DiffusionEquationSolver3D_BC(
    ...
    void DiffusionEquationSolver3D::init(const int nx, const int ny, const int nz)
    {
        ...
        std::vector<std::string> options;
            options.push_back("DiffusionEquation3d");
        ...
        std::cout << "DiffusionEquationSolver3D arguments:" << std::endl;
        for (int i = 1; i < argc; i++) {
            std::cout << options[i] << " ";
        }
        std::cout << std::endl;

        char help[] = "Solves the Diffusion equation in 3D.\n\n";
        ...
        //KSPSetComputeRHS(m_ksp,ComputeRHS_DIFFEQ3D,&m_user);
        //KSPSetComputeOperators(m_ksp,ComputeMatrix_DIFFEQ3D,&m_user);
    ...
    void DiffusionEquationSolver3D::solve(
            std::vector<double> & f,
            const int nx,
            const int ny,
            const int nz,
            ...
            )
    {
        ...
        if (f.size() != nx*ny*nz)
        {
            std::cerr<<"---> DiffusionEquationSolver3D: inconsistent sizes";
        }
...
//printf("\n");
    ...
    double DiffusionEquationSolver3D::solveResidual(
...
PetscErrorCode ComputeRHS_DIFFEQ3D(KSP ksp,Vec b,void *ctx)
{
    ...
    if (user->bc_type.north == DIFFEQ3D_BC_NEUMANN) {
...
PetscErrorCode ComputeMatrix_DIFFEQ3D(KSP ksp,Mat J, Mat jac,void *ctx)
{
    UserContext    *user = (UserContext*)ctx;
    PetscInt       i,j,k,mx,my,mz,xm,ym,zm,xs,ys,zs,num, numi, numj, numk;
    PetscScalar    v[7],DyDzdDx,DxDzdDy,DxDydDz, lambda;
        ...
        for (j=ys; j<ys+ym; j++)
        {
            for (i=xs; i<xs+xm; i++)
            {
                ...
                if (i==0 || j==0 || k==0 || i==mx-1 || j==my-1 || k==mz-1)
                {
                    ...
                    v[num]     = 1+(PetscReal)(numi+numj+numk)*lambda;
                    col[num].i = i;
                    col[num].j = j;
                    col[num].k = k;
                    num++;
                ...
                {
                    v[0] = -lambda;                          col[0].i = i;   col[0].j = j;   col[0].k = k-1;
                    v[1] = -lambda;                          col[1].i = i;   col[1].j = j-1; col[1].k = k;
                    v[2] = -lambda;                          col[2].i = i-1; col[2].j = j;   col[2].k = k;
                    v[3] = 1+6*lambda;                       col[3].i = i;   col[3].j = j;   col[3].k = k;
                    v[4] = -lambda;                          col[4].i = i+1; col[4].j = j;   col[4].k = k;
                    v[5] = -lambda;                          col[5].i = i;   col[5].j = j+1; col[5].k = k;
                    v[6] = -lambda;                          col[6].i = i;   col[6].j = j;   col[6].k = k+1;
